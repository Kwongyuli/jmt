...
//        }
    ...
    // 글 작성
    @PostMapping("/write")
    public String createDesert(@Valid @ModelAttribute DesertCreate desertCreate, @RequestParam("files") MultipartFile[] files) throws IOException {
    ...
    @GetMapping("/{id}/edit")
    public String editDesertForm(@PathVariable Long id, Model model) {
        DesertUpdate desertUpdate = desertService.getDesertUpdate(id);
        desertUpdate.setId(id); // desertUpdate 객체에 id 값 설정
    ...
    // 글 수정
    @PostMapping("/{id}")
    public String updateDesert(@PathVariable Long id, @ModelAttribute DesertUpdate desertUpdate, @RequestParam("files") MultipartFile[] files) throws IOException {
...
@PostMapping("/{id}/upvote")
@ResponseBody
public ResponseEntity<Map<String, Long>> upvoteDesert(@PathVariable Long id) {
    desertService.upvote(id);
    ...
    @PostMapping("/{id}/downvote")
    @ResponseBody
    public ResponseEntity<Map<String, Long>> downvoteDesert(@PathVariable Long id) {
        desertService.downvote(id);
        long upvotes = desertService.getUpvotes(id);
        long downvotes = desertService.getDownvotes(id);
        Map<String, Long> response = new HashMap<>();
